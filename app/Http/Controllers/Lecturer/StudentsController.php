...
class StudentsController extends Controller
{
    ...
    {
        return view('layouts.lecturer.students.index');
    }
        ...
        return response()->json([
            ...
            'data' => $data->map(function ($row, $index) use ($start) {
                ...
                return [
                    ...
                    'action' => '
                    <button class="btn btn-outline-primary btn-sm btn-reset-password" data-id="' . $row->id . '"><i class="fas fa-key"></i> Reset Password</button>
                    ',
                ];
            }),
        ]);
    ...
    public function resetPassword(Request $request, $id)
    {
        ...
        $nim = $user->detail->nim ?? null;
        if (!$nim) {
            return response()->json(['message' => 'NIM not found for this user.'], 422);
        }
